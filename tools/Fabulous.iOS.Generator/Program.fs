...
module Program =
    ...
    let configuration =
        { baseTypeName = "UIKit.UIResponder"
          propertyBaseType = "" } // Not applicable to iOS
        ...
        | Some options ->            
            let result =
                ...
                (*|> Program.withWorkflow (fun workflow ->
                    { workflow with
                        optimize = iOSOptimizers.optimize })
                |> Program.withReadAssembliesConfiguration (fun config ->
                    { config with
                        isTypeResolvable = Reflection.isTypeResolvable
                        tryGetStringRepresentationOfDefaultValue = iOSConverters.tryGetStringRepresentationOfDefaultValue }
                )*)
